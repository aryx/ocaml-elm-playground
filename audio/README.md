...
| `instruments/` (`audio_instruments`) | the live blocks of a synthesizer's voice | `Vco`, `Lfo`, `Drift`, `Voicing`, `Moog_ladder`, `Svf` |
| `effects/` (`audio_effects`) | the live effects, and the rack that chains them | `Effect` (the interface), `Drive`, `Eq`, `Delay`, `Reverb`, `Rack` |
