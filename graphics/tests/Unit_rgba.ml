...
let test_rgb_to_rgba () =
  ...
  let img = Rgba.of_stb_image rgb in
  Alcotest.(check (pair int int)) "size" (64, 64) (img.width, img.height);
  Alcotest.(check int) "bytes" (64 * 64 * 4) (Bigarray.Array1.dim img.rgba);
  (* every pixel: the same r, g, b, and an opaque alpha *)
  for i = 0 to (64 * 64) - 1 do
    for k = 0 to 2 do
      if rgb.data.{(i * 3) + k} <> img.rgba.{(i * 4) + k} then Alcotest.failf "pixel %d, channel %d" i k
    done;
    if img.rgba.{(i * 4) + 3} <> 255 then Alcotest.failf "pixel %d: alpha" i
  done
...
let test_gray_alpha () =
  ...
  | Ok gray ->
      let img = Rgba.of_stb_image gray in
      Alcotest.(check (list int)) "gray 100, alpha 50" [ 100; 100; 100; 50 ]
        (List.init 4 (fun k -> img.rgba.{k}))
