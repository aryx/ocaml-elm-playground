...
type t = { left : line; right : line; mutable delay : float (* in samples, gliding to the setting *) }
...
let create () : t = { left = line (); right = line (); delay = -1. }
...
let process (t : t) (s : settings) (out : Signal.stereo) : unit =
  ...
  Array.iteri
    (fun i x_left ->
      ...
      out.left.(i) <- x_left +. (s.mix *. wet_left);
      out.right.(i) <- x_right +. (s.mix *. wet_right))
    out.left
...
let effect () : Effect.t =
  ...
  { name = "delay"; knobs; set; process = (fun out -> process t !s out) }
