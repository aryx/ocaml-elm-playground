...
| `style/` (`web_style`) | what each element looks like | `Looks` (Mosaic's fixed table, CSS 2.1's appendix D numbers, the user agent's style sheet; and what a declaration does to a look), `Css_syntax` (a style sheet's tokens, blocks and rules: CSS Syntax Level 3, errors skipped), `Selectors` (Level 3: combinators, attributes, pseudo-classes, specificity, matched right to left), `Css` (the cascade by specificity and `!important`, over Mosaic's looks), and TinyChrome's: `Css_values` (lengths, `calc()`, colours, `var()`), `Cascade` (`@media`, origins, the rule index and the ancestor filter), `Computed` (a record of values per element, inherited or initial; the user agent's sheet, `ua.css`) |
| `layout/` (`web_layout`) | where everything goes | `Html_layout` (blocks stacked, margins collapsed, inline content on lines sharing a baseline, broken at the width by a breaker, greedy or the caller's; list markers, anchors; floats; tables), `Table_layout` (a table's grid, and its columns' widths from its cells' minimum and maximum), `Hit` (a point to the link under it, a #fragment's name to its place), and TinyChrome's `Box_layout` (CSS 2.1's box model over `Computed`'s styles: margins, borders, paddings, auto margins, collapsing, floats, inline-blocks shrunk to fit, positioning, tables, lists, flex containers) and `Flex_layout` (flexbox's arithmetic: lines, the room shared by grow and shrink, the items placed) |
