...
| `unix/` (`networking_unix`, native only) | the sockets | `Tcp`, `Udp`, `Server`, `Relay`, `Relay_client`, `Universe_server`, `Irc_server`, `Mail_server` (SMTP in, POP3 out), `Tls_tunnel` (TLS through openssl's s_client, until TLS is ours: Transport.tunnel), `Http_client`, `Http_request`, `Http_server` (a web server's event loop), `Worker` (a pool of threads, for the calls that block: DNS, curl), `Connect` |
