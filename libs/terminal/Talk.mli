(* Talk: the programs of the teletype, which ask and wait, as values.
   ...
   remembered. A frame loop can't stop: the Playground's [update] runs
   once a frame and must return at once, so a game that asks has to
   become a state machine (which question are we on? TinyHamurabi and
   TinyZork keep it in their model). This module gives the BASIC style
   back, without stopping anything: a program is a *value*, the
   conversation written down, and a machine plays it.
   ...
   program never waits; it *is* waiting, as a value, until the machine
   ...
   for when the repository moves to OCaml 5; what it would lose is
   ...
   - **Randomness from a seed**: [Random] is a request like the others,
     answered from a seed the machine keeps (Lehmer's generator, the
     one under Playground.random), so the same seed gives the same
     game, and a test can play it.
   ...
   - **The terminal is somebody else's**: the program writes bytes and
     ...
     and TinyTerminal can run the same program under its shell. The
     Playground's Teletype way draws a machine's screen and gives it the
     keyboard; a real terminal could as well.
   ...
   steps a frame, then lets its caller draw and read the keyboard, and
   carries on at the next frame, as a time-sharing system gives each
   program its slice. And what follows a step is built only when the
   step is taken, which lifts the limit below for a program that steps.
...
(* The machine playing a program: its screen, its tty, the program at
   the point it reached. TinyTerminal's shell steps one of these, the
   Teletype way another. *)
type machine
