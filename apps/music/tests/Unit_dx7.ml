...
let t = Testo.create
let op = Voice_dx7.initial.operators.(0)
let cents (a : float) (b : float) : float = 1200. *. Float.log2 (a /. b)

(* the frequencies: ratios, fine, detune, fixed *)
let test_frequencies () =
  Alcotest.(check (float 1e-9)) "INIT VOICE at A4" 440. (Voice_dx7.frequency op 69);
  Alcotest.(check (float 1e-9)) "coarse 0: an octave down" 220. (Voice_dx7.frequency { op with coarse = 0 } 69);
  Alcotest.(check (float 1e-9)) "coarse 3 fine 50: 4.5 times" 1980. (Voice_dx7.frequency { op with coarse = 3; fine = 50 } 69);
  Alcotest.(check (float 0.01)) "detune +7 at A4 (cents)" 6.80 (cents (Voice_dx7.frequency { op with detune = 14 } 69) 440.);
  Alcotest.(check (float 0.01)) "detune +7 at A1: more cents" 14.69 (cents (Voice_dx7.frequency { op with detune = 14 } 33) 55.);
  Alcotest.(check (float 1e-9)) "fixed, coarse 2: 100 Hz" 100. (Voice_dx7.frequency { op with fixed = true; coarse = 2 } 69);
  Alcotest.(check (float 1e-9)) "at every key" 100. (Voice_dx7.frequency { op with fixed = true; coarse = 2 } 30)
...
let test_scaling () =
  ...
  Alcotest.(check int) "-lin 99, 3 octaves below the break point" (-95) (Voice_dx7.level_scaling lin (39 + 17 - 36));
  (* an exponential curve starts gently: 3 steps an octave away, 15 at
   * three octaves where the linear one is at 95 *)
  Alcotest.(check int) "+exp 99, an octave above" 3 (Voice_dx7.level_scaling lin (39 + 17 + 12));
  Alcotest.(check int) "+exp 99, 3 octaves above" 15 (Voice_dx7.level_scaling lin (39 + 17 + 36));
  Alcotest.(check int) "at the break point: none" 0 (Voice_dx7.level_scaling lin (39 + 17));
  (* the velocity's steps: Dexed's table, 239 its middle *)
  let v = { op with velocity = 7 } in
  Alcotest.(check int) "velocity 127, sensitivity 7: 5.3 dB up" 224 (Voice_dx7.output_level v ~note:60 ~velocity:127 - (127 lsl 5));
  Alcotest.(check int) "velocity 64, sensitivity 7: 10.5 dB down" (-448) (Voice_dx7.output_level v ~note:60 ~velocity:64 - (127 lsl 5));
  Alcotest.(check int) "sensitivity 0: velocity ignored" (127 lsl 5) (Voice_dx7.output_level op ~note:60 ~velocity:1);
  Alcotest.(check int) "rate scaling 7 at C7: 21 more qrate" 21 (Voice_dx7.rate_scaling { op with rate_scaling = 7 } 96)

(* the bytes: every preset's 128 back to it; a cartridge *)
let test_bytes () =
  List.iter
    (fun (name, (p : Voice_dx7.patch)) ->
      let back = Voice_dx7.of_packed (Voice_dx7.to_packed p) in
    ...
    (("initial", Voice_dx7.initial) :: Voice_dx7.presets);
  let voices = Array.of_list (List.map snd Voice_dx7.presets) in
  let syx = Voice_dx7.to_cartridge voices in
  Alcotest.(check int) "a cartridge: 4104 bytes" 4104 (String.length syx);
  (match Voice_dx7.cartridge syx with
  | Ok v ->
  ...
  Alcotest.(check bool) "a byte changed: the checksum says so" true (Voice_dx7.cartridge (Bytes.to_string broken) = Error "the checksum is wrong");
  Alcotest.(check bool) "too short" true (Result.is_error (Voice_dx7.cartridge "\xf0\x43"))

(* the patches as text, and back *)
let test_text () =
  List.iter
    (fun (name, p) ->
      match Voice_dx7.of_string (Voice_dx7.to_string p) with
      | Ok q -> Alcotest.(check bool) (name ^ ": read back") true (q = p)
      | Error e -> Alcotest.failf "%s: %s" name e)
    Voice_dx7.presets;
  let names = List.map (fun (k : Voice_dx7.knob) -> k.name) Voice_dx7.knobs in
...
let play ?(release = 1.) ?(velocity = 1.) (p : Voice_dx7.patch) (keys : int list) (seconds : float) : Signal.t * Voice_dx7.t =
  let v = Voice_dx7.create p in
  let i = Voice_dx7.instrument v in
...
let test_voice () =
  let s, v = play Voice_dx7.initial [ 69 ] 1.5 in
  ...
  Alcotest.(check int) "released: freed" 0 (Voice_dx7.voices v);
  let _, v = play ~release:10. Voice_dx7.initial (List.init 20 (fun k -> 40 + k)) 0.1 in
  Alcotest.(check int) "20 keys: 16 voices" 16 (Voice_dx7.voices v)

(* each preset's peak over the riff, to keep them under clipping *)
let riff (p : Voice_dx7.patch) : Signal.t =
  let v = Voice_dx7.create p in
  let i = Voice_dx7.instrument v in
...
let test_peaks () =
  Alcotest.(check (list (pair string (float 0.01)))) "the presets' peaks"
    ...
    (List.map (fun (name, p) -> (name, peak (riff p))) Voice_dx7.presets)
