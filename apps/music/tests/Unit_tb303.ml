...
let pattern (text : string) : Sequencer.step array =
  match Tb303_voice.pattern_of_string text with Ok p -> p | Error e -> failwith e

(* a voice running [text] at 120 BPM, [samples] of it a sample at a
 * time, [probe] read after each: its values *)
let trace (text : string) (samples : int) (probe : Tb303_voice.t -> float) : float array =
  let v = Tb303_voice.create { Tb303_voice.initial with bpm = 120.; decay = 0.5; pattern = pattern text } in
  let i = Tb303_voice.instrument v in
  Tb303_voice.run v true;
...
let test_decay () =
  ...
  let at_200ms text = (trace text (Signal.samples 0.2) Tb303_voice.envelope).(Signal.samples 0.2 - 1) in
 ...
 * each) *)
let test_sweep () =
  let s = trace "C2* C2* C2* . . ." (3 * 5513) Tb303_voice.sweep in
...
let test_slide () =
  ...
  let p = trace "C2~ C3 . ." (at + 1) Tb303_voice.pitch in
...
let test_gate () =
  let v = Tb303_voice.create { Tb303_voice.initial with bpm = 120.; pattern = pattern "C2 . . ." } in
  let i = Tb303_voice.instrument v in
  Tb303_voice.run v true;
...
let test_text () =
  List.iter
    (fun (name, p) ->
      match Tb303_voice.of_string (Tb303_voice.to_string p) with
      | Ok p' -> if p' <> p then Alcotest.failf "%s: not the same after writing and reading" name
      | Error e -> Alcotest.failf "%s: %s" name e)
    Tb303_voice.presets;
  Alcotest.(check string) "a pattern written" "C2 C2~ C3* . Eb2 F#3*~" (Tb303_voice.pattern_to_string (pattern "C2 C2~ C3* - Eb2 F#3*~"));
  Alcotest.(check bool) "a bad step refused" true (Result.is_error (Tb303_voice.pattern_of_string "C2 H2"))

(* each preset: a bar of 16 steps and a little after, a frame's block at
 * a time *)
let bar (p : Tb303_voice.patch) : Signal.t =
  let v = Tb303_voice.create p in
  let i = Tb303_voice.instrument v in
  Tb303_voice.run v true;
