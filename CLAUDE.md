...
- `kits/` → genre kits, private libraries on top of `elm_playground` shared by the games of a genre (`kits/racing/`: `Road`, `Car`, used by `games/TinyOutRun.ml` and `games3d/TinyVirtuaRacing.ml`; `kits/shmup/`: `Shots`, `Path`, used by `games/TinyInvaders.ml` and `games/TinyGalaga.ml`; `kits/platformer/`: `Tile_move`, `Ladder`, used by `games/TinyMario.ml`, `games/TinyLodeRunner.ml`, `games/TinyRick.ml`); generic layers (`Camera2d`, `Tilemap`, `Sprite`, `Scene2d`) are in `playground/` itself. See `docs/claude_notes/plan_games.md`.
