...
- `games2.5d/` → the pseudo-3D games (`TinyWolf`, `TinyOutRun`, `TinyKart`, `TinyDoom`, `TinyComanche`, and `TinyDescent`, full 3D but drawn by the game itself): a 3D look on the 2D playground, each with its original's trick written out in the game, laid out like `games/` (`js/`, `software/`); `games2.5d/README.md` compares the tricks and lists their `games3d/` twins.
