...
- `games2.5d/` → the pseudo-3D games (`TinyZaxxon`, `TinyDiablo`, `TinyDungeonMaster`, `TinyWolfenstein`, `TinyOutRun`, `TinyKart`, `TinyDoom`, `TinyComanche`, and `TinyDescent`, full 3D but drawn by the game itself): a 3D look on the 2D playground, each with its original's trick written out in the game and marked in it (`grep "the trick of this game" games2.5d/*.ml`), laid out like `games/` (`js/`, `software/`); `games2.5d/README.md` compares the tricks, their sizes and their `games3d/` twins.
