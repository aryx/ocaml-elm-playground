(* Game AI for the playground: what an enemy wants, in one line.
   ...
   behaviours with different numbers. Underneath is Steering.mli
   (Craig Reynolds, 1999) and Flock.mli (Reynolds, 1987), where the
...
(* A way from one tile to another, over whatever grid the game already
   ...
   frontier, no queue (Pathfind.mli has all three written out).
...
(* A computer to play a turn game against, from its rules alone. The
   ...
   (Minimax.mli, Deepening.mli, Mcts.mli) stays behind the
   door.
...
(* A state machine, in the shape a game wants it: a list of changes of
   mind, a mode now, and how long it has been in it (Fsm.mli).
