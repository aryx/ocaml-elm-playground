...
## The folders
    ...
    playground/
      ...
      platforms/  the backends: native/ native_common/ software/ web/ svg/
      tests/
...
| `ways/` | other people's ways of programming, each of which builds the Playground `app` for you, so a program written on one has no `update` and no `view` of its own | `Logo` and `Logo3d` (the turtle), `Bigbang` (HtDP's world programs), `Universe` (its worlds with a mailbox), `Puzzlescript` (a game as a map and rewrite rules), `Karel` (Pattis's robot), `Teletype` (the teletype's games, which ask and wait: a program as a conversation), `Textmode` (the terminal's full-screen programs, curses' bytes to a VT100), `Povray` (POV-Ray's: a scene of solids, CSG, lights, mirrors and glass, ray traced a slice per frame, each of the ray tracer's algorithms kept and shown side by side) |
| `platforms/` | the backends, a library or two each: `native/` (SDL and Cairo or OpenGL), `native_common/` (the SDL loop the native ones share), `software/` (our own rasterizer), `web/` (the browser: vdom, WebGL), `svg/` (the 3D scene drawn as SVG) | a `Playground_platform` and/or `Playground3d_platform` each |
...
folders (under `platforms/`) and `tests/` define their own stanzas, so
they say `(include_subdirs no)`.
...
`app`; the root is for the API itself; `platforms/` is for what a
backend needs. Add it to the 3D stanza's list
if it's 3D.
