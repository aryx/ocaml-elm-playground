...
| `apps/music/Voice_tr808`, `TinyTR808` | the TR-808: drums synthesized (the kick's punch and sigh, bridged-T tones, filtered noise, six squares through two band-passes for the metal, the choke), its 16-step sequencer on `Sequencer`'s clock | §10 | done |
