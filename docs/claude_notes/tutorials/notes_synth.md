...
| `apps/music/Op1_engine` | the OP-1's engines behind four encoders: FM (four operators), cluster (the supersaw), string (a live waveguide), pulse, phase (Casio's phase distortion), digital, dr wave (a wave made in the frequency domain: a brick-wall filter, the phase unheard), voltage (two oscillators, two envelopes), d-synth (a drum), the sampler (Sampler's voice, its recording from the tape) | §10 | done |
...
| `apps/music/Opxy_engine` | four of the OP-XY's own engines on Op1_engine's interface: wavetable (two frames crossfaded, their spectra with them), organ (registrations, a transistor organ's squares), hardsync (the master's period kept), simple (a shape morphing) | §10 | done |
| `apps/music/Studio_opxy`, `TinyOpxy` | the OP-XY: eight tracks on one clock, steps of chords with parameter locks, an OP-1 sound, a kit sampled from our 808 or a sampled Rhodes per track, scenes switched at the bar, the brain transposing by scale degree; the sequencers the clock, each track playing its pattern from the ticks: the step components multiply (a ratchet), pulse, hold, skip | §10 | done |
| `apps/music/TinyReface` | the Reface's four faces over our originals (Voice_hammond, Voice_rhodes, Voice_dx7, Voice_cs80) through Voice.S: each face its labels and knob names, its controls drawn from the knobs' kinds | §10 | done |
| `apps/music/Studio_rebirth`, `TinyReBirth` | ReBirth: two 303s, the 808 and the 909 on one clock (four sample-exact sequencers started on the same sample), a mixer, distortion, delay, compressor, the pattern controlled filter | §10 | done |
| `apps/music/Voice_tr808`, `TinyTR808` | the TR-808: drums synthesized (the kick's punch and sigh, bridged-T tones, filtered noise, six squares through two band-passes for the metal, the choke), its 16-step sequencer on `Sequencer`'s clock; the TR-909 switched in: its kick's falling pitch, its cymbals 6-bit samples (our own recordings in a ROM, played at their tune), shuffle and flam | §10 | done |
