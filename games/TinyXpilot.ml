(* Claude Code
 *
 * Copyright (C) 2026 Yoann Padioleau
 *
 * This library is free software; you can redistribute it and/or
 * modify it under the terms of the GNU Library General Public License
 * (LGPL) as published by the Free Software Foundation; either version
 * 2 of the License, or (at your option) any later version.
 *)
(* A toy version of XPilot (Bjørn Stabell and Ken Ronny Schouten,
 * University of Tromsø, 1991): a ship in a cave, with inertia and
 * gravity, cannons in the walls, fuel stations, and the ball game --
 * fly down to the ball, catch it on your connector, and haul it back to
 * your treasure box, in the floor, the ball swinging under the ship. Three balls home
 * win. Left/right to turn, up to thrust, down for the shield, space to
 * fire, b to let the ball go.
 *
 * XPilot was one of the first games played over the Internet: a server,
 * and clients on the X terminals of the universities' labs, dozens of
 * players in the same cave, teams, robots, and hundreds of maps written
 * as ASCII files, walls and slopes and cannons and fuel -- like [level]
 * below. Its ancestors: Gravitar (Atari, 1982: gravity and caves) and
 * Thrust (Jeremy Smith, 1986, on the BBC Micro and the C64), where the
 * ship hauls a pod on a rod out of a planet: the ball game is Thrust's
 * pod, multiplayer. (Names and dates from memory, to check.)
 *
 * Three ideas in it are new to the games of this directory:
 *
 *  - Newton's third law, by hand: the connector is a rope, a spring
 *    that only pulls, and only when stretched ([rope_pull]); the same
 *    force pulls the ship towards the ball and the ball towards the
 *    ship, the other way ([Physics.push] with opposite signs). The ball
 *    being twice as heavy, it moves half as much (F = m a): feel it
 *    swing the ship when you turn, and drag it down when you stop
 *    thrusting.
 *
 *  - Crashing is a jolt: a ship bouncing off a wall has its velocity
 *    changed at once, and the size of that change says how hard it hit
 *    ([hit_walls]). Landing softly or sliding along a wall changes it a
 *    little, ramming it a lot: above [crash], the ship explodes. What
 *    kills pilots in real life too: not the speed, the deceleration.
 *
 *  - Cannons aim ahead ([intercept]): a shot at speed s meets a target
 *    at d moving at v after a time t with |d + v t| = s t, a quadratic
 *    in t. Fly straight at a steady speed, and the cannons hit you; turn
 *    and thrust, and they miss.
 *
 * What it uses: Tilemap (the map, drawn with view_visible), Camera2d
 * (follow, clamp; the radar is the map again, scaled down), Scene2d
 * (title, play, won), and Physics: the ship ([upright]: a wall never
 * spins it), the ball and the walls are bodies ([immovable] walls,
 * [bounce_off] them, [turn], [thrust], [fall], [push], [shot_from],
 * [touching]); underneath, physics/2d/'s Body, Integrate, Shape,
 * Collide and Resolve. Not a world and its solver (nothing piles up),
 * not Physics.pulled_to (a spring to a fixed point; the rope links two
 * moving bodies), not went_through (the shots, 9 pixels a frame, are
 * slower than a wall is thick). The shots feel no gravity, so that the
 * cannons' aim is exact.
 *
 * The flag hitboxes draws what the physics sees.
 *
 * Exercises: other pilots (robots, then real ones over the network,
 * plan_networking_teaching.md), the enemy team's treasure to defend,
 * XPilot's wormholes, its items (afterburners, missiles, cloaking),
 * the refueling key rather than refueling by just being near, a bigger
 * map, read from a file in XPilot's own format.
 *)
open Playground
open Basics (* float arithmetics *)

(*****************************************************************************)
(* The map *)
(*****************************************************************************)

(* x walls; q w a s slopes, the letter saying which corner of the tile
 * is filled, as the keys sit on a keyboard (q the top-left, s the
 * bottom-right); c cannons, # fuel, * the treasure box, _ the base, o
 * the ball *)
let level : string list =
  [ "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
    "x            wxxxxxxxxxxxxxxq          x";
    "x             wxxxxxxxxxxxxq           x";
    "x              wxxxxxxxxxxq            x";
    "x                                      x";
    "x    _     #                           x";
    "x**xxxxxxxxxxxxxxxxxxa       sxxxxa    x";
    "xxxxxxxxxxxxxxxxxxxxxxa     sxxxxxxa   x";
    "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx   x";
    "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxcxxxx   x";
    "x                                      x";
    "x                                      x";
    "x                                      x";
    "x         sca             sa           x";
    "x        sxxxa           sxxa          x";
    "x       sxxxxxa         sxxxxa         c";
    "x                                      x";
    "x                                      x";
    "x   o                                  x";
    "x  xxx                           #     x";
    "xxxxxxxxxxxxcxxxxxxxxxxxxxxxxxxxxxxxxxxx" ]

let tile = 40.
let h = tile / 2.
let map = Tilemap.of_strings tile level

let is_block (c : char) : bool = match c with 'x' | 'c' | '#' | '*' -> true | _ -> false
let is_slope (c : char) : bool = match c with 'q' | 'w' | 'a' | 's' -> true | _ -> false

(* a slope's triangle, around its tile's center, counterclockwise:
 *
 *    q      w      a      s
 *   +--+   +--+   +      +   (x the filled corner)
 *   |x/     \x|   |\      /|
 *   |/       \|   |x\    /x|
 *   +         +   +--+  +--+
 *)
let triangle (c : char) : (number * number) list =
  let tl = (-.h, h) and tr = (h, h) and bl = (-.h, -.h) and br = (h, -.h) in
  match c with 'q' -> [ tl; bl; tr ] | 'w' -> [ tl; br; tr ] | 'a' -> [ tl; bl; br ] | _ -> [ bl; br; tr ]

(* whether the world point (x, y) is in a wall: a block, or the filled
 * half of a slope (u and v from -1 to 1 across the tile: the diagonal
 * of q and s is v = u, the one of w and a is v = -u) *)
let solid (x : number) (y : number) : bool =
  match Tilemap.tile_at map x y with
  | Some c when is_block c -> true
  | Some c when is_slope c -> (
      let col, row = Tilemap.cell map x y in
      let cx, cy = Tilemap.center map col row in
      let u = (x - cx) / h and v = (y - cy) / h in
      match c with 'q' -> v > u | 's' -> v < u | 'w' -> u + v > 0. | _ -> u + v < 0.)
  | _ -> false

let wall_color = rgb 40 60 190

(* The walls, for the physics and to draw them: each row's runs of
 * blocks as one rectangle (drawn tile by tile, the tiles' edges would
 * show), each slope a triangle; with their half width, to find the ones
 * near a body *)
let walls : (Physics.body * number) list =
  List.concat
    (List.mapi
       (fun row line ->
         let n = String.length line in
         let rec go col acc =
           if col >= n then acc
           else if is_block line.[col] then
             let rec last c = if c +.. 1 < n && is_block line.[c +.. 1] then last (c +.. 1) else c in
             let e = last col in
             let x0, y = Tilemap.center map col row and x1, _ = Tilemap.center map e row in
             let w = x1 - x0 + tile in
             go (e +.. 1) ((Physics.body (rectangle wall_color w tile) |> Physics.at ((x0 + x1) / 2.) y |> Physics.immovable, w / 2.) :: acc)
           else if is_slope line.[col] then
             let x, y = Tilemap.center map col row in
             go (col +.. 1) ((Physics.body (polygon wall_color (triangle line.[col])) |> Physics.at x y |> Physics.immovable, h) :: acc)
           else go (col +.. 1) acc
         in
         go 0 [])
       (Tilemap.to_strings map))

(* the walls in a rectangle *)
let walls_in (r : Camera2d.rect) : Physics.body list =
  List.filter_map
    (fun ((w : Physics.body), half) -> if w.x + half > r.left && w.x - half < r.right && w.y + h > r.bottom && w.y - h < r.top then Some w else None)
    walls

(* the walls near a body: the only ones worth testing *)
let near (b : Physics.body) : Physics.body list = walls_in { left = b.x - 50.; right = b.x + 50.; bottom = b.y - 50.; top = b.y + 50. }

let places (c : char) : (number * number) list = List.map (fun (col, row) -> Tilemap.center map col row) (Tilemap.find map c)
let base = List.hd (places '_')
let ball_home = List.hd (places 'o')
let fuel_stations = places '#'
let treasure = places '*'

(*****************************************************************************)
(* The model *)
(*****************************************************************************)

type ship = {
  body : Physics.body;
  fuel : number;
  dead : int option; (* exploded, n frames ago *)
}

type shot = { shot : Physics.body; ttl : int (* frames left *) }

type cannon = {
  mx : number; (* its muzzle, on the side of its tile facing the cave *)
  my : number;
  facing : number * number;
  reload : int; (* frames before it can fire *)
  down : int; (* frames before it's rebuilt, 0 when it's up *)
}

type game = {
  ship : ship;
  ball : Physics.body;
  connected : bool;
  grab_wait : int; (* frames before the connector can catch the ball again *)
  ball_free : int; (* frames since the ball was let go *)
  shots : shot list; (* the ship's *)
  bullets : shot list; (* the cannons' *)
  cannons : cannon list;
  delivered : int;
  deaths : int;
  frames : int;
  cam : Camera2d.t;
}

type scene = Title | Playing of game | Won of game
type model = scene Scene2d.t

let gravity = 60. (* pixels per second, per second: XPilot's caves pull gently *)
let fuel_max = 150.
let balls_to_win = 3

let ship_shape = polygon white [ (16., 0.); (-12., 11.); (-12., -11.) ]

let new_ship () : ship =
  let x, y = base in
  { body = Physics.body ship_shape |> Physics.at x (y - 4.) |> Physics.pointing 90. |> Physics.upright |> Physics.bouncy 0.4; fuel = fuel_max; dead = None }

let new_ball () : Physics.body =
  let x, y = ball_home in
  Physics.body (circle white 10.) |> Physics.at x y |> Physics.heavy 2. |> Physics.bouncy 0.5

(* a cannon faces the first open side of its tile: below, above, left,
 * right *)
let cannons : cannon list =
  List.mapi
    (fun i (col, row) ->
      let open_ (dc, dr) = match Tilemap.get map (col +.. dc) (row +.. dr) with Some ' ' -> true | _ -> false in
      let dc, dr = List.find open_ [ (0, 1); (0, -1); (-1, 0); (1, 0) ] in
      let fx = float_of_int dc and fy = -.float_of_int dr in
      let x, y = Tilemap.center map col row in
      { mx = x + (fx * h); my = y + (fy * h); facing = (fx, fy); reload = 40 *.. i; down = 0 })
    (Tilemap.find map 'c')

let new_game () : game =
  let x, y = base in
  { ship = new_ship (); ball = new_ball (); connected = false; grab_wait = 0; ball_free = 0; shots = []; bullets = [];
    cannons; delivered = 0; deaths = 0; frames = 0; cam = { (Camera2d.origin |> Camera2d.look_at x y) with zoom = 1.5 } }

let initial_model : model = Scene2d.start Title

(*****************************************************************************)
(* The rope *)
(*****************************************************************************)

let rope_length = 110.
let stiffness = 60. (* the pull per pixel of stretch *)
let rope_damping = 8. (* the pull per pixel per second of stretching *)

(* [rope_pull ship ball]: the force pulling the ship towards the ball
 * (the ball is pulled the other way, as hard): nothing while the rope
 * is slack, then a spring, k times the stretch (Hooke's law), plus a
 * damper resisting the stretching, so that the ball doesn't bounce on
 * its rope forever; and a rope never pushes. E.g. the ball 120 pixels
 * right of the ship, both still: stretched by 10, a pull of 600 to the
 * right on the ship, 600 to the left on the ball -- which, twice as
 * heavy, accelerates at 300. Hanging still under the ship, the ball
 * stretches it by 2 (its weight, 2 x 60, over k). *)
let rope_pull (s : Physics.body) (b : Physics.body) : number * number =
  let dx = b.x - s.x and dy = b.y - s.y in
  let d = Float.hypot dx dy in
  if d <= rope_length then (0., 0.)
  else
    let nx = dx / d and ny = dy / d in
    let stretching = ((b.vx - s.vx) * nx) + ((b.vy - s.vy) * ny) in
    let f = Float.max 0. ((stiffness * (d - rope_length)) + (rope_damping * stretching)) in
    (f * nx, f * ny)

(*****************************************************************************)
(* Update *)
(*****************************************************************************)

let crash = 300. (* a velocity change, in pixels per second, that breaks the ship *)
let thrust_power = 320.
let turn_speed = 250.

(* [hit_walls b]: [b] bounced off the walls it touches, and how hard it
 * hit: the biggest change of its velocity. With the ship's bounciness
 * of 0.4, hitting a wall head on at v changes the velocity by 1.4 v:
 * above 214 pixels per second, a crash. Landing at 60 (after a fall of
 * 30 pixels: v^2 = 2 g d), a jolt of 84, fine; falling from 380 pixels,
 * almost 10 tiles, without thrusting: a crash. *)
let hit_walls (b : Physics.body) : Physics.body * number =
  List.fold_left
    (fun ((b : Physics.body), jolt) w ->
      let (b' : Physics.body) = Physics.bounce_off w b in
      (b', Float.max jolt (Float.hypot (b'.vx - b.vx) (b'.vy - b.vy))))
    (b, 0.) (near b)

let refueling (s : ship) : bool =
  s.dead = None && List.exists (fun (x, y) -> Float.hypot (x - s.body.x) (y - s.body.y) < 110.) fuel_stations

let fly (keys : keyboard) (pull : number * number) (s : ship) : ship =
  let thrusting = keys.kup && s.fuel > 0. and shielding = keys.kdown && s.fuel > 0. in
  let turning = (if keys.kleft then turn_speed else 0.) - if keys.kright then turn_speed else 0. in
  let body, jolt =
    s.body |> Physics.turn turning
    |> Physics.thrust (if thrusting then thrust_power else 0.)
    |> Physics.fall gravity
    |> Physics.push (fst pull) (snd pull)
    |> Physics.step |> hit_walls
  in
  let fuel = s.fuel - (if thrusting then 3. / 60. else 0.) - if shielding then 8. / 60. else 0. in
  let fuel = if refueling s then Float.min fuel_max (fuel + (30. / 60.)) else fuel in
  { body; fuel = Float.max 0. fuel; dead = (if jolt > crash then Some 0 else None) }

(* [intercept dx dy vx vy speed]: when a shot at [speed], fired now from
 * (0, 0), can meet a target at (dx, dy) moving at (vx, vy): the time t
 * with |d + v t| = speed t, squared:
 *   (v.v - speed^2) t^2 + 2 (d.v) t + d.d = 0
 * the smallest positive root, None if the target outruns the shot. E.g.
 * a ship 300 to the right, going up at 160, a shot at 200: t^2 (160^2 -
 * 200^2) + 300^2 = 0, t = 2.5 s, aimed at (300, 400), 500 away: 200 x
 * 2.5 (a 3-4-5 triangle). *)
let intercept (dx : number) (dy : number) (vx : number) (vy : number) (speed : number) : number option =
  let a = (vx * vx) + (vy * vy) - (speed * speed) and b = 2. * ((dx * vx) + (dy * vy)) and c = (dx * dx) + (dy * dy) in
  let roots =
    if Float.abs a < 1e-6 then if b < 0. then [ -.c / b ] else []
    else
      let disc = (b * b) - (4. * a * c) in
      if disc < 0. then [] else [ (-.b - sqrt disc) / (2. * a); (-.b + sqrt disc) / (2. * a) ]
  in
  match List.filter (fun t -> t > 0.) roots with [] -> None | ts -> Some (List.fold_left Float.min infinity ts)

(* nothing of the walls between two points: sampled every 10 pixels *)
let in_sight (x0 : number) (y0 : number) (x1 : number) (y1 : number) : bool =
  let n = int_of_float (Float.hypot (x1 - x0) (y1 - y0) / 10.) in
  List.for_all
    (fun k ->
      let f = float_of_int k / float_of_int n in
      not (solid (x0 + (f * (x1 - x0))) (y0 + (f * (y1 - y0)))))
    (List.init (max 0 (n -.. 1)) (fun k -> k +.. 1))

let bullet_speed = 280.
let bullet_shape = circle orange 4.

(* a cannon fires when it's up, loaded, and sees the ship in front of it,
 * not too far: at where the ship will be *)
let fire_cannon (s : ship) (c : cannon) : cannon * shot list =
  if c.down > 0 then ({ c with down = c.down -.. 1 }, [])
  else if c.reload > 0 then ({ c with reload = c.reload -.. 1 }, [])
  else
    let dx = s.body.x - c.mx and dy = s.body.y - c.my in
    let fx, fy = c.facing in
    let d = Float.hypot dx dy in
    if s.dead <> None || d > 520. || ((dx * fx) + (dy * fy)) / d < 0.2 || not (in_sight c.mx c.my s.body.x s.body.y) then (c, [])
    else
      let tx, ty =
        match intercept dx dy s.body.vx s.body.vy bullet_speed with
        | Some t -> (dx + (s.body.vx * t), dy + (s.body.vy * t))
        | None -> (dx, dy)
      in
      let n = Float.hypot tx ty in
      let b = Physics.body bullet_shape |> Physics.at c.mx c.my |> Physics.moving (bullet_speed * tx / n) (bullet_speed * ty / n) in
      ({ c with reload = 100 }, [ { shot = b; ttl = 240 } ])

let move_shots (l : shot list) : shot list =
  List.filter_map
    (fun s ->
      let b = Physics.step s.shot in
      if s.ttl <= 0 || solid b.x b.y then None else Some { shot = b; ttl = s.ttl -.. 1 })
    l

(* the ship's shots hitting a cannon's tile bring it down for 15 s *)
let hit_cannons (shots : shot list) (cs : cannon list) : cannon list =
  List.map
    (fun c ->
      let fx, fy = c.facing in
      let cx = c.mx - (fx * h) and cy = c.my - (fy * h) in
      let hit (s : shot) = Float.abs (s.shot.x - cx) <= h + 4. && Float.abs (s.shot.y - cy) <= h + 4. in
      if c.down = 0 && List.exists hit shots then { c with down = 900 } else c)
    cs

let update_game (computer : computer) (scenes : model) (g : game) : game =
  let keys = computer.keyboard in
  let pressed k = Scene2d.pressed k scenes in
  (* the ship, pulled by the rope *)
  let pull = if g.connected then rope_pull g.ship.body g.ball else (0., 0.) in
  let ship, deaths =
    match g.ship.dead with
    | Some n when n >= 60 -> (new_ship (), g.deaths)
    | Some n -> ({ g.ship with dead = Some (n +.. 1) }, g.deaths)
    | None ->
        let s = fly keys pull g.ship in
        (s, if s.dead = None then g.deaths else g.deaths +.. 1)
  in
  (* the ball, pulled the other way *)
  let ball, _ = g.ball |> Physics.fall gravity |> Physics.push (-.fst pull) (-.snd pull) |> Physics.step |> hit_walls in
  (* the ship's shots, and the cannons' *)
  let firing = ship.dead = None && pressed (fun k -> k.kspace) && List.length g.shots < 8 && ship.fuel >= 1.5 in
  let shots = move_shots g.shots @ if firing then [ { shot = Physics.body (circle white 3.) |> Physics.shot_from 520. 20. ship.body; ttl = 90 } ] else [] in
  let ship = if firing then { ship with fuel = ship.fuel - 1.5 } else ship in
  let cannons, fired = List.split (List.map (fire_cannon ship) (hit_cannons g.shots g.cannons)) in
  let bullets = move_shots g.bullets @ List.concat fired in
  (* the cannons' shots hitting the ship: stopped by its shield *)
  let hits = List.filter (fun (s : shot) -> ship.dead = None && Physics.touching s.shot ship.body) bullets in
  let shielded = computer.keyboard.kdown && ship.fuel > 0. in
  let bullets = List.filter (fun s -> not (List.memq s hits)) bullets in
  let ship, deaths = if hits <> [] && not shielded then ({ ship with dead = Some 0 }, deaths +.. 1) else (ship, deaths) in
  (* the connector: catching the ball near it, letting it go with b,
   * breaking when stretched too far *)
  let d = Physics.distance ship.body ball in
  let let_go = pressed (fun k -> Set_.mem "b" k.keys) in
  let connected, grab_wait =
    if g.connected then if ship.dead <> None || let_go || d > rope_length + 70. then (false, 60) else (true, 0)
    else (ship.dead = None && g.grab_wait = 0 && d < 60., max 0 (g.grab_wait -.. 1))
  in
  (* the ball touching the treasure box: home! *)
  let home = List.exists (fun (x, y) -> Float.abs (x - ball.x) < h + 11. && Float.abs (y - ball.y) < h + 11.) treasure in
  let ball_free = if connected then 0 else g.ball_free +.. 1 in
  let ball, connected, delivered, ball_free =
    if home then (new_ball (), false, g.delivered +.. 1, 0)
    else if ball_free > 600 then (new_ball (), false, g.delivered, 0) (* left alone 10 s, it goes back to its place *)
    else (ball, connected, g.delivered, ball_free)
  in
  let cam = g.cam |> Camera2d.follow 0.12 ship.body.x ship.body.y |> Camera2d.clamp computer.screen (Tilemap.bounds map) in
  { ship; ball; connected; grab_wait; ball_free; shots; bullets; cannons; delivered; deaths; frames = g.frames +.. 1; cam }

let update (computer : computer) (model : model) : model =
  let scenes = Scene2d.update computer model in
  let space = Scene2d.pressed (fun k -> k.kspace) scenes in
  match scenes.scene with
  | Title -> if space then Scene2d.go (Playing (new_game ())) scenes else scenes
  | Playing g ->
      let g = update_game computer scenes g in
      if g.delivered >= balls_to_win then Scene2d.go (Won g) scenes else { scenes with scene = Playing g }
  | Won _ -> if space then Scene2d.go Title scenes else scenes

(*****************************************************************************)
(* View *)
(*****************************************************************************)

let text (color : color) (size : number) (s : string) : shape = words color s |> scale size

(* a line from (x0, y0) to (x1, y1) *)
let segment (color : color) (width : number) (x0 : number) (y0 : number) (x1 : number) (y1 : number) : shape =
  rectangle color (Float.hypot (x1 - x0) (y1 - y0)) width
  |> rotate (atan2 (y1 - y0) (x1 - x0) * 180. / pi)
  |> move ((x0 + x1) / 2.) ((y0 + y1) / 2.)

(* the tiles drawn over the walls *)
let tile_shape (c : char) : shape =
  match c with
  | '#' -> group [ square (rgb 200 50 40) tile; square (rgb 120 20 20) (tile - 10.); text white 1.5 "F" ]
  | '*' -> group [ square (rgb 230 200 40) tile; square (rgb 140 110 10) (tile - 10.) ]
  | '_' -> rectangle (rgb 60 220 90) tile 5. |> move_y (-.h + 2.5)
  | _ -> group []

let view_cannon (c : cannon) : shape list =
  if c.down > 0 then []
  else
    let fx, fy = c.facing in
    [ circle (rgb 200 200 210) 9. |> move c.mx c.my;
      rectangle (rgb 200 200 210) 16. 5. |> rotate (atan2 fy fx * 180. / pi) |> move (c.mx + (fx * 8.)) (c.my + (fy * 8.)) ]

let view_ship (keys : keyboard) (s : ship) : shape list =
  let b = s.body in
  match s.dead with
  | Some n ->
      let r = 6. + (0.8 * float_of_int n) in
      [ circle orange r |> fade (Float.max 0. (1. - (float_of_int n / 60.))) |> move b.x b.y ]
  | None ->
      let flame = if keys.kup && s.fuel > 0. then [ polygon orange [ (-12., 6.); (-26., 0.); (-12., -6.) ] ] else [] in
      let outline = [ ship_shape; polygon black [ (9., 0.); (-9., 6.); (-9., -6.) ] ] in
      let shield = if keys.kdown && s.fuel > 0. then [ circle (rgb 90 180 255) 26. |> fade 0.35 ] else [] in
      [ group (flame @ outline) |> rotate b.angle |> move b.x b.y; group shield |> move b.x b.y ]

(* the radar: the map again, 1/12 of its size, each row's runs of wall
 * one rectangle *)
let radar_scale = 1. / 12.
let radar_map : shape =
  let t = tile * radar_scale in
  group
    (List.concat
       (List.mapi
          (fun row line ->
            List.filter_map
              (fun col ->
                if is_block line.[col] || is_slope line.[col] then
                  let x, y = Tilemap.center map col row in
                  Some (square (rgb 70 90 200) t |> move (x * radar_scale) (y * radar_scale))
                else None)
              (List.init (String.length line) Fun.id))
          (Tilemap.to_strings map)))

let view_hud (screen : screen) (g : game) : shape list =
  let r = Tilemap.bounds map in
  let rx = screen.right - 20. - (r.right * radar_scale) and ry = screen.top - 20. - (r.top * radar_scale) in
  let dot color (x, y) = circle color 3. |> move (rx + (x * radar_scale)) (ry + (y * radar_scale)) in
  let fuel = g.ship.fuel / fuel_max in
  [ rectangle black ((r.right - r.left) * radar_scale) ((r.top - r.bottom) * radar_scale) |> move rx ry;
    radar_map |> move rx ry;
    dot white (g.ship.body.x, g.ship.body.y);
    dot yellow (g.ball.x, g.ball.y);
    text white 2. "FUEL" |> move (screen.left + 50.) (screen.top - 30.);
    rectangle (rgb 60 60 60) 200. 14. |> move (screen.left + 190.) (screen.top - 30.);
    rectangle (if fuel < 0.2 then red else rgb 220 60 50) (200. * fuel) 14. |> move (screen.left + 90. + (100. * fuel)) (screen.top - 30.);
    text yellow 2.5 (Printf.sprintf "BALLS %d / %d" g.delivered balls_to_win) |> move (screen.left + 110.) (screen.top - 70.);
    text white 2. (Printf.sprintf "DEATHS %d   %.0f s" g.deaths (float_of_int g.frames / 60.)) |> move (screen.left + 110.) (screen.top - 105.) ]

let view_world (computer : computer) (g : game) : shape list =
  let rope = if g.connected then [ segment (rgb 230 230 120) 2. g.ship.body.x g.ship.body.y g.ball.x g.ball.y ] else [] in
  let beam =
    if refueling g.ship then
      List.filter_map
        (fun (x, y) -> if Float.hypot (x - g.ship.body.x) (y - g.ship.body.y) < 110. then Some (segment (rgb 255 120 90) 2. x y g.ship.body.x g.ship.body.y) else None)
        fuel_stations
    else []
  in
  let visible = Camera2d.visible computer.screen g.cam in
  List.map Physics.draw (walls_in visible)
  @ [ Tilemap.view_visible visible tile_shape map ]
  @ List.concat_map view_cannon g.cannons
  @ beam @ rope
  @ [ Physics.draw g.ball ]
  @ view_ship computer.keyboard g.ship
  @ List.map (fun s -> Physics.draw s.shot) (g.shots @ g.bullets)
  @ if List.mem_assoc "hitboxes" computer.flags then List.map Physics.debug ((g.ship.body :: g.ball :: near g.ship.body) @ near g.ball) else []

let view (computer : computer) (model : model) : shape list =
  let screen = computer.screen in
  rectangle black screen.width screen.height
  ::
  (match model.scene with
  | Title ->
      let whole = Camera2d.origin |> Camera2d.look_at 0. 150. in
      [ Camera2d.view { whole with zoom = 0.6 } (List.map (fun (w, _) -> Physics.draw w) walls @ (Tilemap.view tile_shape map :: List.concat_map view_cannon cannons) @ [ Physics.draw (new_ball ()) ])
        |> fade 0.5;
        text white 6. "TINY XPILOT" |> move_y 300.;
        text white 2. "left/right turn   up thrust   down shield   space fire   b let go" |> move_y 220.;
        text yellow 2. "catch the ball down in the cave, set it on your yellow box: three to win" |> move_y 185. ]
      @ Scene2d.blink 1. model [ text yellow 3. "PRESS SPACE" |> move_y (-420.) ]
  | Playing g -> Camera2d.view g.cam (view_world computer g) :: view_hud screen g
  | Won g ->
      (Camera2d.view g.cam (view_world computer g) :: view_hud screen g)
      @ [ text yellow 5. "MISSION COMPLETE" |> move_y 100.;
          text white 3. (Printf.sprintf "%.0f seconds, %d deaths" (float_of_int g.frames / 60.) g.deaths) |> move_y 30. ]
      @ Scene2d.blink 1. model [ text yellow 3. "PRESS SPACE" |> move_y (-100.) ])

(* the keys and flags, printed at launch, to remember them (on the web,
 * in the browser's console) *)
let help =
  {|TinyXpilot
  keys:  left/right  turn
         up          thrust
         down        shield (uses fuel)
         space       fire (start, restart)
         b           let the ball go
  flags: hitboxes    draw what the physics sees
  e.g.   dune exec games/TinyXpilot.exe -- hitboxes
|}

let app = game view update initial_model

let main =
  print_string help;
  Playground_platform.run_app ~flags:(Playground_platform.flags ()) app
