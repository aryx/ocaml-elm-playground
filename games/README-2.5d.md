...
| `TinyFez` | Fez (Phil Fish, Polytron, 2012) | an orthographic view from the side, turned about the vertical by the view's angle: each cube's side faces turned towards the eye, centres rotated, widths `|cos a|` and `|sin a|`, the 8x8 textures squashed to them run by run, far first; at a quarter turn exactly, a flat picture | face (runs of pixels) | 26 / 469 | a top or a bottom (the camera never tilts); a turn that is not about the level's centre; anything but cubes |
