 ...
 *
 * It shares its bones with TinyKickOff2 -- both are the sports
 ...
 * reflection -- see TinyPinball.ml, which does the same thing
 ...
 * (TinyPortal2D's transform, on a ball); the management between
 ...
 * as a pinball bumper is (TinyPinball.ml): push the ball out
...
let update_game (computer : computer) (g : game) : game =
  ...
  if g.restarting > 0 then { g with restarting = g.restarting -.. 1 }
     ...
     * and that is the whole difference with TinyKickOff2. There
