 ...
 * add above it. Doom's trick again (TinyDoom.ml: the nearest
 ...
 * the map stops.
 *
 * As in TinyMarioKart.ml, a "pixel" is 5 x 5 real ones: the view is a
 ...
 *
 * TinyComanche3d.ml flies over the same island, the grid as
...
(* coupling: TinyComanche3d.ml's island and balloons, the same *)
...
(* coupling: TinyComanche3d.ml's model and update, the same *)
type model = {
  x : number;
  y : number;
  alt : number;
...
(* coupling: TinyComanche3d.ml's panel, the same *)
