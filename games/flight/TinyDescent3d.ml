 ...
 *)
(* TinyDescent.ml again, in real 3D: the same mine (the
...
(* coupling: TinyDescent.ml's model and update, the same *)
...
(* coupling: TinyDescent.ml's wall_color, without its fade
...
(* coupling: TinyDescent.ml's panel, with the cells it can't
...
let view (computer : computer) (m : model) : camera * shape3d list =
  ...
  (* 90 degrees over the view of TinyDescent, which is the
