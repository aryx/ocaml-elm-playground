 ...
 *)
(* TinyComanche.ml, the same flight over the same island, but the
...
(* coupling: TinyComanche.ml's island and balloons, the same *)
...
(* coupling: TinyComanche.ml's model and update, the same *)
type model = {
  x : number;
  y : number;
  alt : number;
...
(* coupling: TinyComanche.ml's panel, the same *)
