...
(* a slice of road: its index, its two edges' heights, how much it
 * curves, and what stands beside it (the side, -1 left, 1 right, times
 * the distance from the center, in road widths) *)
type segment = { index : int; y1 : number; y2 : number; curve : number; sprites : (number * roadside) list }

(* easing, for curves and hills that come and go smoothly *)
let ease_in a b p = a +. ((b -. a) *. p *. p)
let ease_in_out a b p = a +. ((b -. a) *. ((-.cos (p *. Float.pi) /. 2.) +. 0.5))

(* A section: [enter] segments easing into the curve and the height,
 * [hold] segments of it, [leave] easing out; [hill] is how much the road
 * rises (or falls) over the section, in segment lengths *)
type section = { enter : int; hold : int; leave : int; curve : number; hill : number }

let straight n = { enter = n / 4; hold = n / 2; leave = n / 4; curve = 0.; hill = 0. }
let curve n c = { enter = n / 4; hold = n / 2; leave = n / 4; curve = c; hill = 0. }
let hill n h = { enter = n / 4; hold = n / 2; leave = n / 4; curve = 0.; hill = h }
let both n c h = { enter = n / 4; hold = n / 2; leave = n / 4; curve = c; hill = h }

let sections =
  [ straight 40; curve 80 2.; hill 80 20.; curve 80 (-4.); both 100 3. 40.; straight 40; both 80 (-2.) (-40.);
    curve 60 5.; curve 60 (-5.); hill 100 30.; both 80 4. (-30.); straight 60; curve 100 (-3.); hill 60 (-20.);
    straight 40 ]

let track : segment array =
  let segs = ref [] and y = ref 0. and index = ref 0 in
  List.iter
    (fun s ->
      let total = s.enter + s.hold + s.leave in
      let y_start = !y and y_end = !y +. (s.hill *. segment_length) in
      let add curve p =
        let y2 = ease_in_out y_start y_end p in
        segs := { index = !index; y1 = !y; y2; curve; sprites = [] } :: !segs;
        y := y2;
        incr index
      in
      for i = 0 to s.enter - 1 do add (ease_in 0. s.curve (float_of_int i /. float_of_int s.enter)) (float_of_int i /. float_of_int total) done;
      for i = 0 to s.hold - 1 do add s.curve (float_of_int (s.enter + i) /. float_of_int total) done;
      for i = 0 to s.leave - 1 do
        add (ease_in_out s.curve 0. (float_of_int i /. float_of_int s.leave)) (float_of_int (s.enter + s.hold + i) /. float_of_int total)
      done)
    sections;
  (* the road ends where it started: a loop, laps *)
  let segs = Array.of_list (List.rev !segs) in
  Array.map
    (fun s ->
      let sprites =
        if s.index mod 8 = 0 then [ (-1.4, Palm); (1.4, Palm) ]
        else if s.index mod 8 = 4 then [ ((if s.index mod 16 = 4 then -1.8 else 1.8), Bush) ]
        else if s.index mod 150 = 75 then [ (1.3, Sign) ]
        else []
      in
      { s with sprites })
    segs

let track_length = float_of_int (Array.length track) *. segment_length
...
let fps = 60.
let max_speed = segment_length *. fps (* one segment per frame *)
let accel = max_speed /. 5.
let brake = -.max_speed
let decel = -.max_speed /. 5.
let off_road_decel = -.max_speed /. 2.
let off_road_limit = max_speed /. 4.
let centrifugal = 0.3

type race = {
  position : number; (* how far along the track, 0 .. track_length *)
  x : number; (* across the road: -1 its left edge, 1 its right *)
  speed : number;
  steer : number; (* -1, 0, 1: for the car's sprite *)
...
let new_race = { position = 0.; x = 0.; speed = 0.; steer = 0.; sky = 0.; time = 0; laps = 0 }
let initial_model : model = Scene2d.start Title

let segment_at (z : number) : segment =
  let n = Array.length track in
  track.((int_of_float (floor (z /. segment_length)) mod n + n) mod n)

let clamp lo hi v = Float.max lo (Float.min hi v)

let update_race (k : keyboard) (r : race) : race =
  let dt = 1. /. fps in
  let percent = r.speed /. max_speed in
  let seg = segment_at r.position in
  let steer = (if k.kleft then -1. else 0.) +. if k.kright then 1. else 0. in
  (* steering, faster at speed; the curve pushes you outwards *)
  let x = r.x +. (steer *. dt *. 2. *. percent) -. (dt *. 2. *. percent *. seg.curve *. centrifugal) in
  let speed =
    if k.kup then r.speed +. (accel *. dt) else if k.kdown then r.speed +. (brake *. dt) else r.speed +. (decel *. dt)
  in
  (* on the grass, down to a crawl *)
  let speed = if Float.abs x > 1. && speed > off_road_limit then speed +. (off_road_decel *. dt) else speed in
  let speed = clamp 0. max_speed speed in
  let position = r.position +. (speed *. dt) in
  let laps = if position >= track_length then r.laps + 1 else r.laps in
  { position = Float.rem position track_length; x = clamp (-3.) 3. x; speed; steer;
...
let view_segment (screen : screen) (n : int) (seg : segment) (p1 : projected) (p2 : projected) : shape list =
...
let view_road (screen : screen) (r : race) : shape list =
  let base = segment_at r.position in
  let percent = Float.rem r.position segment_length /. segment_length in
  ...
  let rec go n x dx clip acc =
    if n >= draw_distance then acc
      ...
      let seg = track.((base.index + n) mod Array.length track) in
  ...
  List.concat_map (fun (n, seg, p1, p2) -> view_segment screen n seg p1 p2 @ List.map (view_sprite screen p1) seg.sprites) visible
...
let view (computer : computer) (s : model) : shape list =
  ...
  let car_shape = (if r.steer < 0. then car |> rotate 3. else if r.steer > 0. then car |> rotate (-3.) else car) |> move 0. (screen.bottom +. 90.) in
  ...
  let hud =
    ...
    | Racing r ->
        [ text white 3. (Printf.sprintf "%3.0f km/h" (r.speed /. max_speed *. 290.)) |> move (screen.right -. 150.) (screen.top -. 40.);
