 ...
 * underneath is still TinyMicroMachines' (the racing kit's
 * Topdown, driven on the plane, the height only drawn) -- that much is
 * shared with TinyMarioKart, where the same car is drawn in Mode
 ...
 *     is TinyMario64's lesson, and free here since every kart
 *     already drags one.
 *
 * Uses: the racing kit's Track3d (the circuit) and Topdown (the car,
 * with TinyMicroMachines and TinyMarioKart), Sprite (its
 ...
 * not crashing (TinyMarioKart does the same) *)
 ...
 * high the kart is (TinyMario64's lesson). *)
 ...
 * screen shows, seen the way TinyMicroMachines draws it for real
