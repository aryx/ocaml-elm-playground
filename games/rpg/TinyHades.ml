 ...
 * TinyRogue.ml (Rogue, 1980), TinyDiablo.ml (Diablo,
 ...
 * TinyBoomerangFu.ml, whose dash is only speed: there, being
 * somewhere else is the whole of the defence.
 *
 * Why it is a 3D game and not a 2.5D one: Hades is drawn in 2D from a
 ...
 * no world in common with it -- and it is a 3D game to be the same
 * genre with the engine doing the drawing: a camera, triangles and a
 * z-buffer, where TinyDiablo.ml has two lines of arithmetic
 ...
 * what TinyDiablo.ml's two lines of arithmetic do by hand *)
