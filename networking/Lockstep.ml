...
type t = {
  ...
  mutable tick : int;
  (* by player: every input known up to this tick (-1: none yet); mine
   * is tick + delay - 1 *)
...
let create ~(me : int) ~(players : int) ~(delay : int) : t =
  ...
  (* the first [delay] ticks: no input, for everybody, known by all *)
  ...
  done;
  {
    ...
    desync = None;
    stats = { stalls = 0; dropped = 0; inputs_sent = 0 };
  }

let tick (t : t) : int = t.tick

let step (t : t) (input : string) : string array option =
  if Array.for_all (fun upto -> upto >= t.tick) t.contiguous then begin
    let inputs = Array.init t.players (fun p -> Hashtbl.find t.inputs (t.tick, p)) in
    Hashtbl.replace t.inputs (t.tick + t.delay, t.me) input;
    t.contiguous.(t.me) <- t.tick + t.delay;
    t.tick <- t.tick + 1;
    Some inputs
  end
  else begin
    t.stats <- { t.stats with stalls = t.stats.stalls + 1 };
    None
  end
...
let packet (t : t) : string =
  ...
  t.stats <- { t.stats with inputs_sent = t.stats.inputs_sent + count };
  Wire.to_bytes (fun w ->
      Wire.put_u8 w 1;
...
let receive (t : t) (bytes : string) : unit =
  ...
  | Ok m when m.from = t.me -> t.stats <- { t.stats with dropped = t.stats.dropped + 1 }
  | Ok m ->
      List.iteri (fun i input -> if not (Hashtbl.mem t.inputs (m.first + i, m.from)) then Hashtbl.replace t.inputs (m.first + i, m.from) input) m.sent;
      (* how far their inputs now go without a hole *)
      while Hashtbl.mem t.inputs (t.contiguous.(m.from) + 1, m.from) do
        t.contiguous.(m.from) <- t.contiguous.(m.from) + 1
      done;
